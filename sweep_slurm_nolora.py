...
def finetune(args):
    ...
    for model in models:
        ...
        for lr in lrs:
            for epoch in epochs:
                job = f"python /xyao/code/trainer/finetune.py --batch-size-per-device {bs} --num-devices {nd} --model_name {model} --output_dir outputs/ --lr {lr} --num-epochs {epoch} --ds-config /xyao/code/trainer/deepspeed_configs/zero_3_llama_2_{model_size}.json --train-path {train_path} --special-token-path {special_token_path} --test-path {test_path}"
                jobs.append(job)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lora-ranks", type=str, default="8")
